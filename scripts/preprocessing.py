...
def train_drop_duplicates(df):
    ...
    idx_first_obj = df_no_doubles.drop(columns='selling_price')\
                                 .drop_duplicates(keep='first').index
    no_d_series = pd.Series(dict((x, x in idx_first_obj)
                                 for x in df_no_doubles.index))
...
def mileage_clean_n_cast(df):
    ...
    df['mileage'] = df['mileage'].map(lambda x: float(str(x)
                                                      .rstrip(' kmpl')
                                                      .rstrip(' km/kg')
                                                      )
                                      )
...
def engine_clean_n_cast(df):
    ...
    df['engine'] = df['engine'].map(lambda x: float(str(x)
                                                    .rstrip(' CC')
                                                    )
                                    )
    return df
...
def max_power_clean_n_cast(df):
    ...
    df['max_power'] = df['max_power'].map(lambda x: str(x)
                                          .rstrip(' bhp')
                                          .rstrip('bhp')
                                          )
    # Заменим '' на nan
    df['max_power'] = df['max_power'].replace(to_replace='^$',
                                              value='nan',
                                              regex=True)
...
def filling_na_with_medians(df, mode):
    ...
    if mode == 'train':
        ...
        # na_replacer = skops.load("preprocessing_models/median_imputer.skops")
        na_replacer = skops.load("preprocessing_models/median_imputer.skops", trusted=True)
    else:
        raise ValueError('Wrong mode value - requires "train"/"infer"')

    transformed = na_replacer.transform(X_nums_na)
    X_filled = pd.DataFrame(data=transformed,
...
def normalize_numerical(X, mode):
    ...
    if mode == 'train':
        ...
        skops.dump(normalizer, "preprocessing_models/normalizer_for_nums.skops")
    elif mode == 'infer':
        # normalizer = skops.load("preprocessing_models/normalizer_for_nums.skops")
        normalizer = skops.load("preprocessing_models/normalizer_for_nums.skops", trusted=True)
        X_num_scaled = normalizer.transform(X_num)
    else:
        raise ValueError('Wrong mode value - requires "train"/"infer"')
...
def encoding_categorical(X, mode):
    ...
    if mode == 'train':
        ...
        # enc = skops.load("preprocessing_models/encoder_for_categorical.skops")
        enc = skops.load("preprocessing_models/encoder_for_categorical.skops", trusted=True)
        X_cat_coded = enc.transform(X_cat).toarray()
    ...
    X_cat_coded_df = pd.DataFrame(X_cat_coded,
